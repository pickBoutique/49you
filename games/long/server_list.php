...
//推荐服务器
    ...
    //所有服务器
	$server_start = strtotime("+2 day",strtotime(date('Y-m-d')));
	$sqlstr="select server_gid,server_id,server_name,server_start,server_isnew,server_ishot from ".DB_PREFIX."server where server_gid = $gid  and (server_status=1 or server_isnew=1 or server_ishot=1) and server_start < $server_start order by server_time desc";
