...
function ChangeUnameInput(atype){
	var aInputObj = gID("login_user");	
	if(aInputObj.value == ''){
		aInputObj.value = "- 请输入账号";
		aInputObj.style.color = "#666666";
	}else if(aInputObj.value == '- 请输入账号'){ 
		aInputObj.value = "";
		aInputObj.style.color = "#000000";	
	}
}

function ChangePwdInput(atype){
	var txtObj = gID("login_pwdTxt");
	var pwdObj = gID("login_pass");
	if (pwdObj.value == ""){
		if (atype == 0) { // 显示框
			txtObj.style.display = "none";
			pwdObj.style.display = "block";
			pwdObj.style.color = "#000000";
			pwdObj.focus();
		} else if (atype == 1) { // 输入框
...
//ajax_ultimate("http://jtxm.joy400.com/ajax_loginsta3.php","getlogin","get");
  ...
  // alert('dsadww');return;
	if(data){
		...
		if(data.servernum){
			...
			$d("oldgame").innerHTML="<a href='http://long.joy400.com/server/' target='_blank' style='color:red'>快点我</a>";
		}
	
	//ajax_ultimate("http://jtxm.joy400.com/ajax_loginsta3.php","getlogin","get","",data);
...
function getlogin(string,data)
{
	...
	//loadContent('http://www.joy400.com/?m=lgame&action=lgame&game_id=24','sendData_callback',0);
}