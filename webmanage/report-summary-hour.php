<?php
define('PERMI_CODE','report_summary_hour_msg');
...
group by sd_sumtime)a";
	...
	$totalrow=array("sd_sumtime"=> "合计","member_free"=>0,"member_recom"=>0,"sd_advtype"=>0,"sd_advid"=>0,"sd_metrid"=>0,"member_metrid"=>0,"member_total"=>0);
	foreach($rs as $k=>$v){
		$rs[$k]['sd_sumtime'] = date("Y-m-d H",$rs[$k]['sd_sumtime']) . '('.$week[date("l", $rs[$k]['sd_sumtime'])] .')'; //日期转换为星期格式
	...
	$page_nav = "统计分析 >> 广告注册统计（时）";
	include_once("templates/report-summary-hour.html");
