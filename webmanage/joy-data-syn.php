	...
	if(!empty($sumdate_star)){
		...
		if(($sumdate_end-$sumdate_star)/(3600*24) > 31){
			...
			for($i=$sumdate_star;$i<=$sumdate_end;$i+=3600*24){
				$date_sta=$i;
				$date_end=$i+3600*24;
				trans_syn($sumact,$date_sta,$date_end);
				echo "<br/>日期：".date("Y-m-d",$i)." 成功";
			}
		}
	}else{
		$date_sta=strtotime(date("Y-m-d H:0:0"));
		$date_end=$date_sta+3600;
		trans_syn($sumact,$date_sta,$date_end);
	}
}
fun_endrun();
...
function game_syn($sumact=''){
	...
	if(empty($sumact) || $sumact=="game"){
		...
		//$db->query($sqlstr);
		$sqlstr="SELECT game_id,title game_name,factory game_code,createtime game_time,description game_desc,url game_web,bbsurl game_bbs,recommend game_recom,rate game_rate
,1 game_status
from game";
...
,createtime trans_addtime
,payment_id trans_type
,ip trans_ip
,status trans_instatus
,intime trans_intime
,outtime trans_outtime
,gamestatus trans_outstatus
,webchannel_id trans_advtype
,webads_id trans_advid
,material_id trans_metrid
,siteman trans_subtype
,register trans_register
FROM paycord
where status = 1 and intime>= {$date_sta} and intime<{$date_end}";
