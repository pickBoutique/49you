...
function sumtotal($sumdatetime,$sumact='',$cln=1){
	...
	if($sumact=="advmetpopup"){
		if(!empty($cln)){
			$sqlstr="delete from ".DB_PREFIX."sumd_advmet_popup where sd_sumtime = $sumdatetime";
...
and c.sd_metrid = b.sd_metrid)";
	...
	if($sumact=="advpopup"){
		if(!empty($cln)){
			$sqlstr="delete from ".DB_PREFIX."sumd_adv_popup where sd_sumtime = $sumdatetime";
			$db_admin->query($sqlstr);
		}
		$sqlstr="select adv_id from ".DB_PREFIX."adv";
		$rsadv=$db_admin->getAll($sqlstr);
...
GROUP BY sd_advtype,sd_advid,sd_ip) a
group by sd_advtype,sd_advid";
		//exit($sqlstr);
			$trs = $db_daywan->getAll($sqlstr);
			if($trs){
				$tmptable=retsqltable($trs,&$strField);
				$sqlstr="update ".DB_PREFIX."sumd_adv_popup a 
right join ".$tmptable."
...
,a.sd_status4 = b.sd_status4";
				...
				$sqlstr="insert into ".DB_PREFIX."sumd_adv_popup({$strField})
select * from ".$tmptable."
where not exists (select * from ".DB_PREFIX."sumd_adv_popup c where c.sd_sumtime=b.sd_sumtime
and c.sd_advtype = b.sd_advtype
and c.sd_advid = b.sd_advid)";
