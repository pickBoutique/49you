<?php
define('PERMI_CODE','report_serverreg_hour');
...
if($act == 'dataget'){
	...
	if($dateset == ''){$where .= "and ss_sumtime>=".(strtotime(date("Y-m-d")));}
...
order by pf_sid desc,ss_sid desc";
	...
	$sqlstr="select date_format(from_unixtime(ss_sumtime),'%Y-%m-%d %H') ss_sumtime
...
//exit($sqlstr);
	...
	$page_nav = "统计分析 >> 服务器注册统计(时)";
	include_once("templates/report-serverreg-hour.html");
