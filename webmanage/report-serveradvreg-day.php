<?php
define('PERMI_CODE','report_serveradvreg_day');
...
if($act == 'dataget'){
	...
	//print_r($db_pd);
	//exit();
	if($dateset == ''){$where .= "and sd_sumtime>=".(strtotime(date("Y-m-d")));}
	
	$sersql="select CONCAT(sd_platform,'_',sd_sid) pf_sid,sd_gid,sd_sid
from ".DB_PREFIX."sumd_adv_server
where 1 {$where}
and sd_memreg > 0
...
order by pf_sid desc,sd_sid desc";
	...
	if($trs){
		...
		$sqlstr="select date_format(from_unixtime(sd_sumtime),'%Y-%m-%d') sd_sumtime
,sd_adv
...
//exit($sqlstr);
	...
	if($rs){
		foreach($rs as $k=>$v){
			...
			$rs[$k]['sd_sumtime'] = $rs[$k]['sd_sumtime'] . '('.$week[date("l", strtotime($rs[$k]['sd_sumtime']))] .')'; //日期转换为星期格式
	...
	$page_nav = "统计分析 >> 服务器广告注册(日)";
	include_once("templates/report-serveradvreg-day.html");
